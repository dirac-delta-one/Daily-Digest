...
set LOGFILE=logs\midday_%LOGDATE%.log
echo [%date% %time%] Starting midday check >> %LOGFILE%
"%~dp0.venv\Scripts\python.exe" midday.py >> %LOGFILE% 2>&1
if %ERRORLEVEL% NEQ 0 "%~dp0.venv\Scripts\python.exe" "%~dp0run_alert.py" midday >> %LOGFILE% 2>&1
